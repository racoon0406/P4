...
const bit<16> TYPE_INDEX = 0x0802;
//idle time in microseconds = abs(d1-d2) in topology.json
const bit<48> delta = 2000;
...
register<bit<32> >(5) reg_pkt_count;
register<bit<48> >(1) reg_pkt_time;
...
/*
 * This is a custom protocol header for the normal packet. We'll use
 * ethertype 0x0802 for is (see parser)
 */
header index_t {
    bit<32>  pkt_index;
    bit<16>  protocol;  
...
struct metadata {
    ...
    bit<32> pkt_count_total;
    //to read packet index so far
    bit<32> index;  
    //to read last ecmp_select the packet chose
    bit<32> last_select;
    //to read last timestamp the packet was sent
    bit<48> last_time;
}
...
parser MyParser(packet_in packet,
    ...
    state parse_index{
        packet.extract(hdr.index);
        transition select(hdr.index.protocol) {
            TYPE_IPV4: parse_ipv4;
            default: accept;
        }
     }
...
control MyIngress(inout headers hdr,
    ...
    action set_ecmp_select(bit<16> ecmp_base, bit<32> ecmp_count) {
        /* TODO: hash on index of the packet and save the hash result in meta.ecmp_select
           so that the ecmp_nhop table can use it to make a forwarding decision accordingly */
        //read packet index so far into local variable
        reg_pkt_count.read(meta.index, 3);   

        //read last time the packet was sent
        reg_pkt_time.read(meta.last_time, 0);

        //read last ecmp_select the packet chose
        reg_pkt_count.read(meta.last_select, 4);

        //two paths for load balancer s1   
        if(ecmp_count != 1)
        {
            if(standard_metadata.ingress_global_timestamp - meta.last_time > delta)
            {
                //switch the path
                if(meta.last_select == 0)
                {
                    meta.ecmp_select = 1;
                    meta.last_select = 1;
                }
            ...
            {
                meta.ecmp_select = meta.last_select;
            }
            meta.last_time = standard_metadata.ingress_global_timestamp;
        }
        else
        {
            meta.ecmp_select = 0;
        ...
        //update register
        reg_pkt_time.write(0, meta.last_time); 
        reg_pkt_count.write(4, meta.last_select);   
        ...
        {               
            ...
            else //For normal packet, reocrd the number of bytes
            {
                ...
                //set index header  
                hdr.index.pkt_index = meta.index;
        ...
        reg_pkt_count.write(3, meta.index);    
